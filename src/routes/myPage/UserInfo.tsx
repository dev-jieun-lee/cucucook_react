...
const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ isOpen, closeModal }) => {
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={closeModal} contentLabel="Change Password" style={modalStyles}>
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <Button color="primary" variant="contained" type="submit" fullWidth sx={userInfoStyles.button}>
...
const UserInfo = ({ isDarkMode }: { isDarkMode: boolean }) => {
  ...
  return (
    <Box sx={userInfoStyles.container}>
      ...
      <Button variant="outlined" color="primary" onClick={() => setIsModalOpen(true)} fullWidth sx={userInfoStyles.button}>
      ...
      <Button color="primary" variant="contained" fullWidth sx={userInfoStyles.button} onClick={handleSaveChangesClick}>
        {t("menu.mypage.save_changes")}
      </Button>

      <Button variant="outlined" color="error" fullWidth onClick={() => window.confirm("정말 회원 탈퇴를 하시겠습니까?") && navigate("/")}>
